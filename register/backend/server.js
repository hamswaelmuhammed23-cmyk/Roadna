...
// ── View engine — EJS (used for admin server-rendered pages) ──────────────────
...
// ── Admin page routes (SSR — MUST come before static-file middleware) ─────────
...
const PREFERRED_PORT = parseInt(process.env.PORT, 10) || 5002
const MAX_PORT_TRIES = parseInt(process.env.PORT_MAX_TRIES, 10) || 10
const PORT_AUTO      = process.env.PORT_AUTO !== "false"
  ...
  const bind = () => {
    ...
    server.listen(listenPort, () => {
      if (listenPort !== PREFERRED_PORT) {
        console.warn(`[Server] Listening on ${listenPort} instead of ${PREFERRED_PORT}`)
      }
      console.log(`Roadna backend running on http://localhost:${listenPort}`);
      console.log(`Health check: http://localhost:${listenPort}/api/v1/health`);
      console.log(`Socket.IO ready on ws://localhost:${listenPort}`);
      console.log(`Admin panel: http://localhost:${listenPort}/admin?token=<JWT>`);
    });
  }
